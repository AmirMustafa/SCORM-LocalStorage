...
var sectionlabel = "Course:";
var promptlabel = "Question:";
...
function renderEntrytoDOM( parentcontainer, entry, sectionid, entryid ) {
    ...
    response.value = entry.response;
    response.setAttribute("onkeyup", "inputData()");      // added for calling inputData() function - amir 15.03.2019
...
// Adding latest localStorage data to db - amir 15.03.2019
function inputData() {
  let count = 0;
  setTimeout(function(){
       ...
       if (key.match(/LearningJournal/)) {
         ...
         $.ajax({
           ...
           success: function (res) {
               // alert(res);
               // res.setItem(key, localStorageJSON);
           }
       });
       count++;
    }
